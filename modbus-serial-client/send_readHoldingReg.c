...
int initialise_serial(const char tty_path[], int baud, int read_timeout_ds) {
  ...
  tty.c_cc[VTIME] = read_timeout_ds; // Wait for up to 1s (10 deciseconds), returning as soon
...
int main() {
  ...
  int serial_port_fd = initialise_serial("/dev/ttyS4",B9600,10);
  if(serial_port_fd<0){
    return 1;
  }
  
  // Write to serial port
  const uint8_t raw_msg[] = {1,3,0,0,0,2};
  const int raw_msg_len = sizeof(raw_msg)/sizeof(raw_msg[0]);
  uint8_t *msg = malloc(raw_msg_len+2*sizeof(uint8_t));
  memcpy(msg, raw_msg,raw_msg_len);
  uint16_t CRC = nmbs_crc_calc(raw_msg,raw_msg_len);
  msg[raw_msg_len] = (uint8_t)(CRC>>8);
  msg[raw_msg_len+1] = (uint8_t)(CRC & 0xFF);
  ...
  int num_bytes = read(serial_port_fd, &read_buf, sizeof(read_buf));
